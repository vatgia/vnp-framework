(function() {
    ...
    function f(a) {
        var b = a.editor,
            c = b.config.simpleuploads_maximumDimensions,
            d = a.data.image;
    ...
    function b(b, c, d, f) {
        if (B) alert("Please, wait to finish the current upload");
        else {
            A = !c;
            v = b;
                        ...
                        window.setTimeout(function() {
                            ...
                            a.onchange = function() {
                                ...
                                if (typeof v.fire("simpleuploads.startUpload", a) != "boolean")
                                    if (a.requiresImage && !CKEDITOR.plugins.simpleuploads.isImageExtension(v, a.name)) alert(v.lang.simpleuploads.nonImageExtension);
                                    else {
                                        F && F.start && F.start(a);
                                        B = this.value;
                ...
                if (!y) {
                    ...
                    y.addEventListener("change", function() {
                        ...
                        if (a) {
                            ...
                            for (var b = 0; b < a; b++) {
                                var c = y.files[b],
                                    d = CKEDITOR.tools.extend({}, y.simpleUploadData);
                                d.file = c;
                                d.name = c.name;
    ...
    function e(a, b) {
        ...
        if (F) {
            ...
            if (a) {
                ...
                c.insertElement(d);
                A && v.fire("simpleuploads.finishedUpload", {
                    name: B,
                    element: d
                })
            }
    ...
    function n(b, c) {
        ...
        i.onload = function() {
            var a = i.responseText.match(/\((?:"|')?\d+(?:"|')?,\s*("|')(.*?[^\\]?)\1(?:,\s*(.*?))?\s*\)\s*;?/),
                ...
                f = a && a[3],
                h = c.id,
                j = b.document.getById(h);
            if (f) {
                var k = f.match(/function\(\)\s*\{(.*)\}/);
                if (k) f = new Function(k[1]);
                else {
                    k = f.substring(0, 1);
        ...
        i.onabort = function() {
            if (c.callback) c.callback.upload(null);
            else {
    ...
    function o(a, b) {
        if (!b.callback) {
            ...
            if (!b.forceLink && c && d) c = l(b.file, b.id, a);
            else {
    ...
    function p(a) {
        ...
        if (d = a.data && a.data.$.clipboardData || b.config.forcePasteAsPlainText && window.clipboardData)
            if (CKEDITOR.env.gecko && b.config.forcePasteAsPlainText && d.types.length === 0) b.on("beforePaste", function(a) {
                a.removeListener();
                a.data.type = "html"
            });
            else {
    ...
    CKEDITOR.plugins.add("simpleuploads", {
        lang: ["en",
            "ar", "cs", "de", "es", "fr", "he", "hu", "it", "ja", "ko", "nl", "pl", "pt-br", "ru", "tr", "zh-cn"
        ],
        onLoad: function() {
        ...
        init: function(a) {
            ...
            if (!e.filebrowserUploadUrl && !e.filebrowserImageUploadUrl) {
                ...
                CKEDITOR.dialog.prototype.showThrobber = function() {
                    if (!this.throbber) this.throbber = {
                        update: function() {
                        ...
                        center: function() {
                            var a = this.mainThrobber,
                                b =
                                this.throbberCover,
                                c = (b.$.offsetHeight - a.$.offsetHeight) / 2;
                ...
                a.ui.addButton && a.ui.addButton("addImage", {
                ...
                if (typeof FormData != "undefined") {
                    ...
                    var F = function(b) {
                            ...
                            if (d && d.files && d.files.length > 0) {
                                ...
                                }, e = 0; e < d.files.length; e++) {
                                    var f = d.files[e],
                                        g = CKEDITOR.tools.getNextId();
        ...
        afterInit: function(a) {
            (a = (a = a.dataProcessor) && a.htmlFilter) && a.addRules(x, {
                applyToAll: true
            })
        }
    });
                ...
                if (c = b.element) {
                    if (e == 1) {
                        ...
                        if (c.getName() == "a") {
                            ...
                            for (; h &&
                                h.nodeType == document.ELEMENT_NODE && h.nodeName.toLowerCase() != "a";) h = h.parentNode;
                ...
                if (b.mode.dialog) {
                    ...
                    var d = function() {
                        if (a.getSelection().getRanges().length)
                            if (a.editable().$.querySelector("#cke_pastebin")) window.setTimeout(d, 0);
                            else {
                                a.fire("updateSnapshot");
                                a.insertElement(c);
                                a.fire("updateSnapshot");
            ...
            if (o(a, b)) {
                ...
                if (!e) {
                    ...
                    if (d.rangeParent) {
                        ...
                        e.setStart(f, j);
                        e.collapse(true)
                    } else if (document.caretRangeFromPoint) e = i.caretRangeFromPoint(f.clientX, f.clientY);
                ...
                if (g == 1) {
                    ...
                    if (i == "a") {
                        ...
                        } else g = e.parentElement(); if (!g || h.nodeName.toLowerCase() ==
                            "img") g = h;
